...
#include "JoustDefenseHistoryWidget.generated.h"
 ...
 */
UCLASS()
class JOUST_API UJoustDefenseHistoryWidget : public UUserWidget
{
	GENERATED_BODY()

public: // ########## public 함수 블록 ##########

	/** 수비 기록을 LanceBox 위의 마커로 갱신 */
	void SetHistory(const TArray<FJoustDefenseHistory>& InHistory, const FVector2D& InLanceBoxMin, const FVector2D& InLanceBoxMax);
...
private: // ########## private 함수 블록 ##########
	...
	/** 수비 결과에 대응하는 마커 모양 반환 */
	FString ResolveDefenseMarkerShape(const FJoustDefenseResult& InDefenseResult) const;

	/** 수비 결과의 표시 이름 반환 */
	FString ResolveDefenseResultText(const FJoustDefenseResult& InDefenseResult) const;

	/** Canvas에 수비 마커 추가 */
	void AddMarker(const FVector2D& InPosition, const FString& InLabel, const FLinearColor& InColor, const FString& InShape, bool bInOverlapMarker);
